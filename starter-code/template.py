        ...
        if self.api_key:
            try:
                ...
                response = client.chat.completions.create(
                    ...
                    messages=[{"role": "user", "content": f"Bạn là chatbot tư vấn du lịch. Hãy trả lời câu hỏi sau của khách hàng mà KHÔNG dùng tool hay internet: {user_input}"}]
                )
                return {"answer": response.choices[0].message.content, "tool_calls": [], "status": "success", "mode": "live_api"}
            except Exception:
                pass
